...
def select_difficulty():
	...
	hard_button = tk.Button(difficulty_window, text="Difícil", command=lambda: set_difficulty("hard"))
...
def game_window():
	...
	def playComputer():
		...
		if difficulty == "easy":
			while True:
				r = random.randrange(1, 10)  # Cambiado para evitar índice fuera de rango
				if board[r] == " ":
	...
	for i in range(3):
		for j in range(1, 4):
			button = Button(frame2, text=" ", width=4, height=2, font=("Arial", 30), bg=bg_color, relief=RAISED,
			                borderwidth=5)
			button.grid(row=i, column=j)
			button.bind("<Button-1>", play)
			buttons.append(button)
	
	restartButton = Button(frame2, text="Reiniciar", width=19, height=1, font=("Arial", 20), bg="Green", relief=RAISED,
	                       borderwidth=5, command=resetGame)
	restartButton.grid(row=4, column=1, columnspan=3)
	
	exitButton = Button(frame2, text="Salir", width=19, height=1, font=("Arial", 20), bg="red", relief=RAISED,
	                    borderwidth=5, command=exit_game)
	exitButton.grid(row=5, column=1, columnspan=3)
	
	score_label = Label(frame1,
	                    text=f"Jugador: {player_score} - Computadora: {computer_score} - Empates: {empate_score}",
...
start_button = tk.Button(start_window, text="Iniciar Juego", command=select_difficulty, font=("Helvetica", 12),
                         bg="yellow")
